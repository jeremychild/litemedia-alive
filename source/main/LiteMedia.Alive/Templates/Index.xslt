<xsl:stylesheet version="2.0"
  ...
  xmlns:a="http://litemedia.info/2011/12"
    ...
    <html>
      ...
      <body>
        <article>
          ...
          <!--<div class="crap">
            <xsl:value-of select="//alive:Groups" />
          </div>-->
          <xsl:apply-templates select="//a:Groups/a:Model.Group" />
        </article>
        <footer>
          <p>
      ...
      <script>
        ...
        var settings = {
          columns: '<xsl:apply-templates select="//a:Settings/a:Columns" />',
          base_color: '#606060',
  ...
  <xsl:template name="configuration">
    charts = {
      <xsl:apply-templates select="//a:Groups/a:Model.Group/a:Charts/a:Model.Chart" mode="json"/>
    }
  </xsl:template>
